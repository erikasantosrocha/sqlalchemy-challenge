...
def precipitation():
    ...
    previous_year_date = dt.date(year_date.year, year_date.month, year_date.day)

    results = session.query(Measurement.date, Measurement.prcp).filter(Measurement.date >=previous_year_date).order_by(Measurement.date).all()
    
    year_dict = dict(results)

    print(f"Results for Precipitation - {year_dict}")
